<template>
  <div class="pt-2 pt-md-4 pb-4 px-2">
    <div class="my-3 pb-2">
      <div class="d-flex flex-wrap justify-content-between align-items-center mb-2">
        <div class="d-flex flex-grow-1 justify-content-start align-items-start mb-3">
          ...
          <div>
            ...
            >
              <circle cx="4" cy="4" r="4" :fill="`${isethereumCoreOperational ? '#00CD98' : '#F6B900'}`" />
            </svg>
            <small v-if="isethereumCoreOperational" class="ml-1 text-success">Running</small>
            <small v-else class="ml-1 text-warning">Starting</small>
            <h3 class="d-block font-weight-bold mb-1">ethereum Node</h3>
            <span class="d-block text-muted">{{
              version ? `ethereum Core ${version}` : "..."
            }}</span>
          </div>
        </div>
      ...
      <b-alert :show="showReindexCompleteAlert" variant="warning">Reindexing is now complete. Turn off "Reindex blockchain" in <span class="open-settings" @click="() => $bvModal.show('advanced-settings-modal')">advanced settings</span> to prevent reindexing every time ethereum Node restarts.</b-alert>
    ...
    <b-alert :show="showRestartError" variant="danger" dismissible @dismissed="showRestartError=false">
      Something went wrong while attempting to change the configuration of ethereum Node.
    </b-alert>
    </div>
    ...
    <b-modal id="advanced-settings-modal" size="lg" centered hide-footer scrollable>
      <advanced-settings-modal :isSettingsDisabled="isRestartPending" @submit="saveSettingsAndRestartethereum" @clickRestoreDefaults="restoreDefaultSettingsAndRestartethereum"></advanced-settings-modal>
    </b-modal>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      isethereumCoreOperational: state => state.ethereum.operational,
      syncPercent: state => state.ethereum.percent,
      blocks: state => state.ethereum.blocks,
      version: state => state.ethereum.version,
      currentBlock: state => state.ethereum.currentBlock,
      blockHeight: state => state.ethereum.blockHeight,
      stats: state => state.ethereum.stats,
      peers: state => state.ethereum.peers,
      rpc: state => state.ethereum.rpc,
      p2p: state => state.ethereum.p2p,
      reindex: state => state.user.ethereumConfig.reindex,
      network: state => state.user.ethereumConfig.network,
      pruned: state => state.ethereum.pruned,
      pruneTargetSizeGB: state => state.ethereum.pruneTargetSizeGB,
      torProxy: state => state.user.ethereumConfig.torProxyForClearnet
    }),
  ...
  methods: {
    ...
    fetchStats() {
      this.$store.dispatch("ethereum/getStats");
    },
    fetchPeers() {
      this.$store.dispatch("ethereum/getPeers");
    },
    fetchConnectionDetails() {
      return Promise.all([
        this.$store.dispatch("ethereum/getP2PInfo"),
        this.$store.dispatch("ethereum/getRpcInfo")
      ]);
    },
    fetchethereumConfigSettings() {
      this.$store.dispatch("user/getethereumConfig");
    },
    async saveSettingsAndRestartethereum(ethereumConfig) {
      try {
        this.isRestartPending = true;
        this.$store.dispatch("user/updateethereumConfig", ethereumConfig);
  
        const response = await API.post(
          `${process.env.VUE_APP_API_BASE_URL}/v1/ethereumd/system/update-ethereum-config`,
          { ethereumConfig }
        );

        if (response.data.success) {
          // reload the page to reset all state and show loading view while ethereum core restarts.
          this.$router.push({ query: { restart: "1" } });
          window.location.reload();
        } else {
          this.fetchethereumConfigSettings();
    ...
    async restoreDefaultSettingsAndRestartethereum() {
      try {
        this.isRestartPending = true;
        
        const response = await API.post(
          `${process.env.VUE_APP_API_BASE_URL}/v1/ethereumd/system/restore-default-ethereum-config`
          );
        
        // dispatch getethereumConfig after post request to avoid referencing default values in the store.
        this.$store.dispatch("user/getethereumConfig");
  ...
  async created() {
    // fetch settings first because ethereum core
    // is not operational if pruning is in progress
    this.fetchethereumConfigSettings();

    // wait until ethereum core is operational
    while (true) { /* eslint-disable-line */
      await this.$store.dispatch("ethereum/getStatus");
      if (this.isethereumCoreOperational) {
        break;
      }
      await delay(1000);
    }
    this.$store.dispatch("ethereum/getVersion");
    this.fetchStats();
    this.fetchPeers();
