const RpcClient = require('ethereumd-rpc');
const camelizeKeys = require('camelize-keys');
const ethereumdError = require('models/errors.js').ethereumdError;

const ethereumD_RPC_PORT = process.env.RPC_PORT || 8332; // eslint-disable-line no-magic-numbers, max-len
const ethereumD_HOST = process.env.ethereum_HOST || '127.0.0.1';
const ethereumD_RPC_USER = process.env.RPC_USER;
const ethereumD_RPC_PASSWORD = process.env.RPC_PASSWORD;

const rpcClient = new RpcClient({
  protocol: 'http',
  user: ethereumD_RPC_USER, // eslint-disable-line object-shorthand
  pass: ethereumD_RPC_PASSWORD, // eslint-disable-line object-shorthand
  host: ethereumD_HOST,
  port: ethereumD_RPC_PORT
});
  ...
  return new Promise((resolve, reject) => {
    try {
      rpcFn.call(rpcObj, (err, info) => {
        if (err) {
          reject(new ethereumdError(`Unable to obtain ${what}`, err));
        } else {
