const ethereumdService = require('services/ethereumd.js');
const ethereumdError = require('models/errors.js').ethereumdError;

async function getBlockCount() {
  const blockCount = await ethereumdService.getBlockCount();

  return {blockCount: blockCount.result};
}

async function getConnectionsCount() {
  const peerInfo = await ethereumdService.getPeerInfo();
...
async function getStatus() {
  try {
    await ethereumdService.help();

    return {operational: true};
  } catch (error) {
    if (error instanceof ethereumdError) {
...
async function getMaxSyncHeader() {
  const peerInfo = (await ethereumdService.getPeerInfo()).result;
...
async function getMempoolInfo() {
  return await ethereumdService.getMempoolInfo();
}

async function getLocalSyncInfo() {
  const info = await ethereumdService.getBlockChainInfo();
...
async function getVersion() {
  const networkInfo = await ethereumdService.getNetworkInfo();
...
async function getTransaction(txid) {
  const transactionObj = await ethereumdService.getTransaction(txid);
...
async function getBlock(hash) {
  const blockObj = await ethereumdService.getBlock(hash);
...
const memoizedGetFormattedBlock = () => {
  ...
  return async blockHeight => {
    ...
    if (blockHeight in cache) {
      ...
      let blockHash;
      try {
        ({result: blockHash} = await ethereumdService.getBlockHash(blockHeight));
      } catch (error) {
        if (error instanceof ethereumdError) {
          return error;
        }
      ...
      const {result: block} = await ethereumdService.getBlock(blockHash);
...
async function getBlockHash(height) {
  const getBlockHashObj = await ethereumdService.getBlockHash(height);
...
async function nodeStatusDump() {
  const blockchainInfo = await ethereumdService.getBlockChainInfo();
  const networkInfo = await ethereumdService.getNetworkInfo();
  const mempoolInfo = await ethereumdService.getMempoolInfo();
  const miningInfo = await ethereumdService.getMiningInfo();
...
async function stop() {
  const stopResponse = await ethereumdService.stop();
  return {stopResponse};
}
